...
export function uniformPickN(arr, n) { return [...arr].sort(() => Math.random() - 0.5).slice(0, Math.min(n, arr.length)); }
...
export function buildCast(protName, protLast, ethnicityBroad, familyStructure, tables, protagonistGenderId, protagonistOrientation, relStatusId, includeLGBQ = true) {
  ...
  function _buildParent(role, structure) {
    ...
    return { name:`${firstName} ${lastName}`, role:isMother?'mother':'father', status:status.label, gender:isMother?'Female':'Male', race:ethnicityBroad, traits:pickTraits(2), dynamic };
  }
...
const GENDER_LABELS = { man:'Male', trans_man:'Trans male', woman:'Female', trans_woman:'Trans female', non_binary:'Non-binary', genderfluid:'Genderfluid' };
...
export function buildSkeleton(stats, mbti, tables, options = {}) {
  ...
  const genderPool   = prefGender !== 'any'
    ? G.filter(g => g.id === prefGender)
    : (includeLGBQ ? G : G.filter(g => g.id === 'man' || g.id === 'woman'));
  const orientPool   = prefOrientation !== 'any'
    ? O.filter(o => o.id === prefOrientation)
    : (includeLGBQ ? O : O.filter(o => o.id === 'straight'));
  ...
  const profPool     = (() => {
    const isNB = gender.id === 'non_binary' || gender.id === 'genderfluid' || gender.id === 'genderless';
  ...
  const relStatusPool = (isMinor && !tables.ALLOW_MINOR_MARRIAGE)
    ...
    : RS;
  let   relStatus          = uniformPick(relStatusPool.flatMap(r => Array(r.weight).fill(r)));
  ...
  if (syntheticType === 'industrial') {
    ...
    orientation = O.find(o => o.id === 'asexual') ?? { id: 'asexual', label: 'Asexual' };
    relStatus   = RS.find(r => r.id === 'single')  ?? { id: 'single',  label: 'Single'  };
  }
  ...
  const cast     = buildCast(name, lastName, identity.broad, famStruct, tables, gender.id, orientation.label, relStatus.id, includeLGBQ);
