...
function GENERIC_OUTPUT_RULES(sk, voice) {
  const openingNote    = voice.openingNote ?? 'Second person. Drop the player into a vivid, specific moment — right now, mid-scene. Sensory detail. End mid-moment with a clear next decision.';
  const appearanceRaw  = voice.appearanceNote;
  const appearanceNote = typeof appearanceRaw === 'function' ? appearanceRaw(sk) : (appearanceRaw ?? 'Comma-separated visual descriptors for a text-to-image model. Start with "portrait of" then describe the subject. No sentences — descriptors only.');
...
export function buildPrompt(sk, voice = {}) {
  ...
  const rules = voice.outputRules != null
    ? (typeof voice.outputRules === 'function' ? voice.outputRules(sk) : voice.outputRules)
...
export function parseResponse(raw) {
  try {
    ...
    const npcEntries = {};
    for (const [k, v] of Object.entries(p.npcEntries ?? {})) npcEntries[k] = coerceEntryString(v);
    return {
    ...
    console.error('parseResponse failed:', e.message, '\nRaw:', raw?.slice(0, 300));
    return null;
  }
}
