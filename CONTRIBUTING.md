...
web/
  ...
  serve.ps1                       ← Same thing for Windows (pure PowerShell — no Python/bash needed)
                                    serve.sh opens a browser when run from a terminal;
                                    --open / --no-browser override that
...
tools/
  ...
  make-mac-icon.sh                ← Rebuilds FateVend.app's .icns from the favicon artwork
...
Start-FateVend.command            ← macOS double-click launcher (runs web/serve.sh --open)
FateVend.app/                     ← macOS app bundle; opens Terminal on Start-FateVend.command
  Contents/MacOS/FateVend         ← Shell script, not a binary — there is nothing to build
  Contents/Resources/FateVend.icns ← Generated by tools/make-mac-icon.sh
```
...
| `tools/make-mac-icon.sh` | Rebuild `FateVend.app`'s icon after the favicon artwork changes (macOS) |
